...
/* ══════════════════════════════════
   MUSIC PLAYER WIDGET
   بيشتغل بموسيقى YouTube عبر Web Audio API
   أو ملف محلي في assets/audio/eid-music.mp3
══════════════════════════════════ */
(function(){
  const audio  = $('bgAudio');
  ...
  const mwBtn  = $('mwBtn');
  const mwEq   = $('mwEq');
  const hdrBtn = $('audioBtn'); // header icon button
  let   playing = false;

  // Show widget after loader disappears
  setTimeout(() => {
  ...
  function togglePlay() {
    ...
    if (playing) {
      audio.pause();
      playing = false;
      mwBtn.querySelector('i').className = 'fa-solid fa-play';
      mwBtn.classList.remove('playing');
      if (mwEq) mwEq.classList.remove('active');
      if (hdrBtn) { hdrBtn.querySelector('i').className='fa-solid fa-music'; hdrBtn.classList.remove('on'); }
    } else {
      // Try to play the local file first
      const playPromise = audio.play();
      if (playPromise !== undefined) {
        playPromise.then(() => {
          playing = true;
          mwBtn.querySelector('i').className = 'fa-solid fa-pause';
          mwBtn.classList.add('playing');
          if (mwEq) mwEq.classList.add('active');
          if (hdrBtn) { hdrBtn.querySelector('i').className='fa-solid fa-volume-high'; hdrBtn.classList.add('on'); }
        }).catch(() => {
          // No local file — use a public domain Eid nasheeed via Web Audio oscillator
          playGeneratedMusic();
        });
      }
    }
  ...
  // ── Procedural music fallback (Web Audio API) ──
  // Generates a simple pleasant looping tone when no MP3 is present
  let audioCtx = null, oscillators = [], gainNode = null;
  ...
  function playGeneratedMusic() {
    try {
      ...
      // Simple pleasant chord — D major arpeggio
  ...
  // Override toggle for generated music
  ...
  if (mwBtn)  mwBtn.addEventListener('click', smartToggle);
  if (hdrBtn) hdrBtn.addEventListener('click', smartToggle);

})();

/* ══════════════════════════════════
   MOBILE: Fix viewport height (100svh fallback)
══════════════════════════════════ */
(function(){
  function setVh() {
...
/* ══════════════════════════════════
   AUTO-SCROLL: Pause on touch (mobile)
══════════════════════════════════ */
(function(){
  const track = $('productsTrack');
  if (!track) return;
